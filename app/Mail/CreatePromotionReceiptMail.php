...
class CreatePromotionReceiptMail extends Mailable
{
    ...
    {
        return new Envelope(
            subject: "Promotion invoice #{$this->invoice->invoice_number} Receipt",
        );
    }
    ...
    {
        return new Content(
            markdown: 'emails.receipts.promotion',
            with: [
    ...
    public function attachments(): array
    {
        ...
        $template = $markdown->render(
            ...
            [
                ...
                'paid_at'           => $this->transaction->paid_at->format('jS F Y h:i'),
                'payment_method'    => $this->transaction->payment_method,
                'purpose'           => $this->invoice->source_type == 'subscription' ? 'Payment for subscription' : 'Payment for advert promotion'
            ]              
        );      
